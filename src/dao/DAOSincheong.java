...
public class DAOSincheong {
	...
	public Vector<ELecture> getItems(ELecture selectedItem) throws IOException {
		...
		// ELecture형 벡터배열 생성, data/sincheong파일 읽는 Scanner생성
		
		FileWriter fw = new FileWriter("data/sincheong", true);
		// data/sincheong파일에 내용을 이어서 입력하는 FileWriter생성
		while(scanner.hasNext()) {
			eItem = new ELecture();
			eItem.read(scanner);
			eItems.add(eItem);
		}
		// ELecture클래스 생성, data/sincheong파일에 있는 강좌정보를 ELecture의 속성에 입력
		// ELecture클래스에 입력된 강좌 정보들을 벡터 배열에 추가
		eItems.add(selectedItem);
		// sincheong에 입력하려는 강좌 정보를 벡터 배열에 추가
		...
		// data/sincheong파일에 줄바꿈 이후, sincheong에 입력하려는 강좌 정보를 추가
